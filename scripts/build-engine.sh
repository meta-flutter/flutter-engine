...
mode=${2:?mode: release|profile|debug|debug-unopt}
...
# `debug-unopt` is --runtime-mode=debug + --unoptimized, output under a
# linux_debug_unopt_<cpu> dir; the plain modes map straight through.
case "$mode" in
  debug-unopt) runtime_mode=debug; mode_dir=debug_unopt; unopt=1 ;;
  release|profile|debug) runtime_mode="$mode"; mode_dir="$mode"; unopt= ;;
  *) echo "build-engine: unknown mode '$mode' (release|profile|debug|debug-unopt)" >&2; exit 2 ;;
esac
...
build_phase() {
  ...
  if [ -n "${EMB_PATCH_DIR:-}" ]; then
    ...
    # series line: "<patch> [subdir] [strip]" — subdir is the -d apply dir
    # (default the engine-src root, "."), strip is -p<n> (default 1). This lets
    # a repo keep patches that git apply reads from a different root: e.g. paths
    # rooted at the gclient solution (a/engine/src/...) apply here with strip 3.
    if [ -f "$EMB_PATCH_DIR/series" ]; then
      while read -r pf pdir pstrip _; do
        [ -z "$pf" ] && continue
        case "$pf" in \#*) continue ;; esac
        local pd="${pdir:-.}" ps="${pstrip:-1}"
        if patch -p"$ps" -d "$pd" -R --dry-run --force \
             < "$EMB_PATCH_DIR/$pf" >/dev/null 2>&1; then
          log "  skip $pf (already applied)"
          continue
        fi
        log "  apply $pf${pdir:+ (in $pdir)}${pstrip:+ -p$pstrip}"
        patch -p"$ps" -d "$pd" --force --no-backup-if-mismatch \
          < "$EMB_PATCH_DIR/$pf"
  ...
  local outdir="out/linux_${mode_dir}_${linux_cpu}"
  ...
  local -a gnargs=(
    --runtime-mode="$runtime_mode" --embedder-for-target --no-build-embedder-examples
